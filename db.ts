...
import type { Course, CourseTextContent, QuizAttempt, QuizData } from "./types";
...
// Course Text Content
export async function addCourseTextContent(
  courseId: number,
  title: string,
  textContent: string
): Promise<number> {
  const db = await getDb();
  return db.add("courseTextContents", {
    courseId,
    title,
    textContent,
    createdAt: new Date(),
...
export async function getCourseTextContents(
  courseId: number
): Promise<CourseTextContent[]> {
  const db = await getDb();
  return db.getAllFromIndex("courseTextContents", "courseId", courseId);
}

export async function getCourseTextContentById(
  id: number
): Promise<CourseTextContent | undefined> {
  const db = await getDb();
  return db.get("courseTextContents", id);
}

export async function updateCourseTextContent(
  id: number,
  title: string,
  textContent: string
): Promise<void> {
...
export async function deleteCourseTextContentsByCourseId(
  courseId: number
): Promise<void> {
...
export async function getQuizStructureById(
  id: number
): Promise<(QuizData & { id: number }) | undefined> {
  const db = await getDb();
  return db.get("quizzes", id) as Promise<
    (QuizData & { id: number }) | undefined
  >;
}

export async function deleteQuizStructuresByCourseId(
  courseId: number
): Promise<void> {
  if (courseId === 0 || courseId === undefined) {
    // Handle quizzes not tied to a specific course if necessary, or skip
    console.log(
      "Skipping deletion of quiz structures not associated with a specific courseId or courseId is 0/undefined."
    );
    return;
  }
...
export async function getQuizAttemptsForCourse(
  courseId: number
): Promise<QuizAttempt[]> {
  const db = await getDb();
...
export async function deleteQuizAttemptsByCourseId(
  courseId: number
): Promise<void> {
  if (courseId === 0 || courseId === undefined) {
    // Handle attempts not tied to a specific course or courseId is 0/undefined
    console.log(
      "Skipping deletion of quiz attempts not associated with a specific courseId or courseId is 0/undefined."
    );
    return;
  }
