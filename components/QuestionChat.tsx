...
${question.answerOptions
  .map(
    (opt) => `- ${opt.text} (Rationale for this option hints: ${opt.rationale})`
  )
  .join("\n")}
`;

    if (
      userSelectedAnswerText !== undefined &&
...
${
  !isUserAnswerCorrect && correctAnswerText
    ? `- The actual correct answer was: "${correctAnswerText}" (Rationale for correct answer: ${
        question.answerOptions.find((opt) => opt.text === correctAnswerText)
          ?.rationale || "N/A"
      })`
    : ""
}
...
`;
  ...
  useEffect(() => {
    ...
    if (!API_KEY) {
      setError(
        "No API key provided. Please provide your Gemini API key to use the chat feature."
      );
      console.error("No API key provided for chat.");
      return;
    }
    setError(null);
    setIsLoading(true);
    ...
    try {
      ...
      let initialModelMessage = `Hello! I'm here to help you with the question: "${question.question.substring(
        0,
        50
      )}...". `;
      if (userSelectedAnswerText !== undefined) {
        initialModelMessage += `You answered "${userSelectedAnswerText}", which was ${
      ...
      setChatHistory([
        { role: "model", text: initialModelMessage, id: Date.now().toString() },
      ]);
    } catch (e) {
      console.error("Failed to initialize chat:", e);
      setError(
        e instanceof Error ? e.message : "Failed to initialize chat service."
      );
    } finally {
      setIsLoading(false);
    }
    ...
    isUserAnswerCorrect,
    correctAnswerText,
  ]); // System instruction depends on these
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    setChatHistory((prev) => [
      ...prev,
      { role: "user", text: messageToSend, id: userMessageId },
    ]);
    setUserInput("");
    setIsLoading(true);
    setError(null);
    ...
    setChatHistory((prev) => [
      ...prev,
      { role: "model", text: "Thinking...", id: modelMessageId },
    ]);
  ...
  if (!API_KEY && !error) {
    return (
      <ErrorMessage message="No API key provided. Chat feature is unavailable." />
    );
  }
  ...
  return (
    <div className="flex flex-col h-[70vh] max-h-[600px] bg-slate-800 rounded-lg shadow-xl animate-fadeInUp">
      <header className="p-4 border-b border-slate-700 flex justify-between items-center">
        <h3
          className="text-lg font-semibold text-sky-300 truncate"
          ...
          >
            <div
              className={`max-w-[80%] p-3 rounded-xl shadow prose prose-sm prose-invert prose-p:my-1 prose-ul:my-1 prose-li:my-0.5
                ${
                  msg.role === "user"
                    ? "bg-purple-600 text-white rounded-br-none prose-strong:text-white"
      ...
      <form
        ...
        className="p-4 border-t border-slate-700 flex items-center gap-3"
      >
        <textarea
          value={userInput}
          ...
          placeholder={
            isLoading ? "Waiting for response..." : "Ask about the question..."
          }
