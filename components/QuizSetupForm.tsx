...
export const QuizSetupForm: React.FC<QuizSetupFormProps> = ({
  ...
  const [generationSource, setGenerationSource] = useState<"topic" | "course">(
    "topic"
  );
  ...
  useEffect(() => {
    const fetchCourseContents = async () => {
      if (selectedCourseId) {
        try {
          const contents = await db.getCourseTextContents(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (generationSource === "topic" && topic.trim()) {
      ...
      if (courseContents.length === 0) {
        alert(
          "Selected course has no text content. Please add content to the course first or choose 'General Topic'."
        );
        return;
      }
      const combinedContextText = courseContents
        .map((cc) => `Content Title: ${cc.title}\n${cc.textContent}`)
        .join("\n\n---\n\n");
      onGenerateQuiz(
        selectedCourse.name,
        numQuestions,
    ...
    <form onSubmit={handleSubmit} className="space-y-6 animate-fadeIn">
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setGenerationSource(e.target.value as "topic" | "course")
          }
        ...
        <div>
          ...
          {selectedCourseId && courseContents.length > 0 && (
            <p className="text-xs text-slate-400 mt-1">
              Using {courseContents.length} text content block(s) from this
              course.
            </p>
          )}
          {selectedCourseId &&
            ...
            generationSource === "course" && (
              <p className="text-xs text-yellow-400 mt-1">
                This course has no text content. Add content in the Course
                Details.
              </p>
            )}
        </div>
      ...
      <button
        ...
        disabled={
          generationSource === "course" &&
          (!selectedCourseId ||
            (courseContents.length === 0 && !!selectedCourseId))
        }
      >
        Generate Quiz
      </button>
