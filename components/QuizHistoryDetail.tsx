...
export const QuizHistoryDetail: React.FC<QuizHistoryDetailProps> = ({
  attempt,
  onBack,
  onRetakeQuiz,
}) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <button
        onClick={onBack}
        className="text-sm text-purple-300 hover:text-purple-200 mb-2"
      >
        ...
        <section>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: marked.parse(attempt.aiSummary) as string,
            }}
          />
        </section>
      )}
          ...
          {attempt.userAnswers.map((ua, index) => (
            ...
            >
              <p className="font-medium text-slate-200 mb-1">
                Q{index + 1}: {ua.questionText}
              </p>
              <div
                className={`flex items-center mb-1 ${
                  ua.isCorrect ? "text-green-400" : "text-red-400"
                }`}
              >
                {ua.isCorrect ? (
              ...
              {!ua.isCorrect && (
                <p className="text-sky-300 mb-1">
                  Correct answer: "{ua.correctAnswerText}"
                </p>
              )}
              {ua.rationale && (
                <p className="text-xs text-slate-400 mt-1 italic">
                  Rationale for your choice: "{ua.rationale}"
                </p>
              )}
            </li>
          ))}
