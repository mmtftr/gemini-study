...
import { generateQuizQuestions, summarizeQuizPerformance } from '../services/geminiService';
import * as db from '../db';
import type { QuizData, Question, AnswerOption, UserAnswer, Course, QuizAttempt } from '../types';
...
export const useQuizManagement = ({
  ...
  const [currentQuizSourceCourseId, setCurrentQuizSourceCourseId] = useState<number | undefined>(undefined);
  const [isQuizGenerationComplete, setIsQuizGenerationComplete] = useState<boolean>(false);
  const [aiPerformanceSummary, setAiPerformanceSummary] = useState<string | null>(null);
  const [isSummaryLoading, setIsSummaryLoading] = useState<boolean>(false);

  const resetQuizState = useCallback((maintainNumQuestions = false, maintainModel = false) => {
    setQuizTopic('');
  ...
  const handleQuizGeneration = useCallback(async (topic: string, questionsCount: number, model: GeminiModel, contextText?: string, courseId?: number) => {
    ...
    try {
      for await (const question of generateQuizQuestions(topic, questionsCount, model, contextText)) {
        allGeneratedQuestions.push(question);
        setDisplayedQuestions(prev => [...prev, question]); // Stream to UI
        setLoadingMessage(`Generated question ${allGeneratedQuestions.length} of ${questionsCount}...`);
      ...
      if (allGeneratedQuestions.length === 0) {
        setError('No questions were generated. The model might have had an issue or the topic/content was too restrictive. Try a different topic or model.');
      ...
      console.error(err);
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred while generating the quiz stream.';
      setError(errorMessage);
      if (allGeneratedQuestions.length > 0) {
        setGeneratedQuizData({ topic, questions: allGeneratedQuestions, modelUsed: model, numQuestionsRequested: questionsCount, courseId });
        if (!switchedToPlayingInLoop) setGameState(GameState.PLAYING);
      } else {
        setGameState(currentQuizSourceCourseId ? GameState.COURSE_DETAIL : GameState.SETUP);
      }
    } finally {
      setIsQuizGenerationComplete(true);
       if (allGeneratedQuestions.length > 0 && allGeneratedQuestions.length < questionsCount) {
        ...
        setError(`Note: Generated ${allGeneratedQuestions.length} out of ${questionsCount} requested questions. The quiz will proceed with the generated ones.`);
      }
      setLoadingMessage('');
    }
  }, [resetQuizState, setGameState, setError, setQuizTopic, setNumQuestionsState, setSelectedModelGlobal, setLoadingMessage, currentQuizSourceCourseId]);

  const handleAnswerSelect = useCallback((answer: AnswerOption) => {
  ...
  const handleNextQuestion = useCallback(async () => {
    ...
    if (currentQuestionIndex < displayedQuestions.length - 1) {
      ...
      if (!isQuizGenerationComplete && displayedQuestions.length < currentNumQuestions) {
        setGameState(GameState.PLAYING);
        setLoadingMessage(`Waiting for question ${displayedQuestions.length + 1} of ${currentNumQuestions}...`);
      } else {
        ...
        setError(null);
        try {
          if (!displayedQuestions || displayedQuestions.length === 0) throw new Error("No questions available for summary.");
  ...
  }, [currentQuestionIndex, displayedQuestions, userAnswers, score, currentSelectedModel, isQuizGenerationComplete, currentNumQuestions, generatedQuizData, setGameState, setLoadingMessage, setError]);

  const handleSaveAndExit = useCallback(async (isPartialExit = false) => {
    ...
    if (!generatedQuizData || !generatedQuizData.questions.length) {
      ...
      // Fix: Use currentGameState prop
      setGameState(currentCourse ? GameState.COURSE_DETAIL : GameState.COURSE_LIST);
      return;
    }
    ...
    try {
      ...
      if (!quizStructureId) {
        ...
        setGeneratedQuizData(prevData => prevData ? { ...prevData, id: quizStructureId } : null);
      }
      ...
      if (currentQuizSourceCourseId && summaryToSave) {
        ...
        await db.addCourseTextContent(currentQuizSourceCourseId, analysisTitle, analysisContent);
      }
      ...
      if (currentQuizSourceCourseId) {
        ...
        if (currentCourse && currentCourse.id === currentQuizSourceCourseId) {
            ...
            const sourceCourse = await db.getCourseById(currentQuizSourceCourseId);
            if (sourceCourse) {
  ...
  }, [generatedQuizData, score, userAnswers, aiPerformanceSummary, currentSelectedModel, quizTopic, currentQuizSourceCourseId, currentCourse, resetQuizState, setGameState, setLoadingMessage, setError, navigateToCourseList, loadCourseDetails, setSelectedCourse]);
  ...
  const handleRetakeQuiz = useCallback(async (attempt: QuizAttempt) => {
    setLoadingMessage("Loading quiz for retake...");
    setError(null);
    try {
      const quizStructure = await db.getQuizStructureById(attempt.quizId);
      if (!quizStructure || !quizStructure.questions || quizStructure.questions.length === 0) {
        setError("Could not find the original quiz questions. It might have been deleted or was incomplete.");
        setLoadingMessage("");
        return;
      }
      ...
      setError(err instanceof Error ? err.message : "An error occurred while trying to retake the quiz.");
    }
    setLoadingMessage("");
  }, [resetQuizState, setQuizTopic, setNumQuestionsState, setSelectedModelGlobal, setGameState, setLoadingMessage, setError]);
  ...
  }, [generatedQuizData, resetQuizState, setQuizTopic, setNumQuestionsState, setSelectedModelGlobal, setGameState, setError]);
  ...
  }, [currentGameState, displayedQuestions, currentQuestionIndex, setGameState, setPreviousGameState]);

  const handleEndChat = useCallback((newPreviousState?: GameState) => { // Allow overriding previous state if needed
  ...
  const handleExitQuizRequest = useCallback(() => {
    openModal({
        title: "Exit Quiz",
        message: "Are you sure you want to exit the quiz? If you've answered any questions, your progress so far will be saved. If not, the quiz attempt won't be saved.",
