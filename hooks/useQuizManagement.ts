    ...
    if (currentQuestionIndex < displayedQuestions.length - 1) {
      ...
      if (
        ...
        try {
          ...
          // Collect previous analyses from the same course if available
          let previousAnalyses: string[] = [];
          if (currentQuizSourceCourseId) {
            try {
              const courseContents = await db.getCourseTextContents(
                currentQuizSourceCourseId
              );
              const performanceAnalyses = courseContents.filter(
                (content) =>
                  content.title
                    .toLowerCase()
                    .includes("performance analysis") ||
                  content.title.toLowerCase().includes("quiz analysis")
              );
              previousAnalyses = performanceAnalyses
                ...
                .slice(-3); // Last 3 analyses to avoid overwhelming the prompt
            } catch (err) {
              console.warn("Could not fetch previous analyses:", err);
            }
          }
    ...
    async (isPartialExit = false) => {
      ...
      try {
        ...
        if (currentQuizSourceCourseId && summaryToSave) {
          ...
          const analysisContent = `# Quiz Performance Analysis

**Course:** ${currentCourse?.name || "Unknown Course"}
**Quiz Topic:** ${quizTopic}
**Date:** ${new Date().toLocaleString()}
**Score:** ${score}/${userAnswers.length} (${Math.round(
            (score / userAnswers.length) * 100
          )}%)
**Model Used:** ${currentSelectedModel}
...
*This analysis was automatically generated by the AI tutor to help track learning progress and identify areas for improvement.*`;
